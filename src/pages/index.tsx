  ...
  return (
    <Container>
      <ContainerHeader>
        ...
        {locale === "pt" ? (
          <h1 data-aos="fade-up">
            {PrismicDom.RichText.asText(textHeader.data.titlept)}
          </h1>
        ) : (
          <h1 data-aos="fade-up">
            {PrismicDom.RichText.asText(textHeader.data.titleen)}
          </h1>
        )}

        {locale === "pt" ? (
          <p data-aos="fade-up">
            {PrismicDom.RichText.asText(textHeader.data.descriptionpt)}
          </p>
        ) : (
          <p data-aos="fade-up">
            {PrismicDom.RichText.asText(textHeader.data.descriptionen)}
          </p>
        )}
      ...
      <ContainerContent>
        <Content>
          {skills.map((skill) => (
            <Item key={skill.uid} data-aos="fade-up">
              <img src={skill.data.image.url} alt="teste" height="200" />
              {locale === "pt" ? (
                <strong>
                  {PrismicDom.RichText.asText(skill.data.titlept)}
                </strong>
              ) : (
                <strong>
                  {PrismicDom.RichText.asText(skill.data.titleen)}
                </strong>
              )}
              {locale === "pt" ? (
        ...
        <Experiences>
          {experiences.map((experience) => {
            if (Number(experience.uid) % 2 === 0) {
              return (
                <ExperienceContent key={experience.uid} data-aos="fade-up">
                  <DescriptionExperience>
                    {locale === "pt" ? (
                      <h1>
                        {PrismicDom.RichText.asText(experience.data.titlept)}
                      </h1>
                    ) : (
                      <h1>
                        {PrismicDom.RichText.asText(experience.data.titleen)}
                      </h1>
                    )}
                    {experience.data.content.map((content) => (
                      <div key={content.id}>
                        {locale === "pt" ? (
                          <h2>
                            {PrismicDom.RichText.asText(content.subtitlept)}
                          </h2>
                        ) : (
                          <h2>
                            {PrismicDom.RichText.asText(content.subtitleen)}
                          </h2>
                        )}
                        {locale === "pt" ? (
                          <p>
                            {PrismicDom.RichText.asText(content.descriptionpt)}
                          </p>
                        ) : (
                          <p>
                            {PrismicDom.RichText.asText(content.descriptionen)}
                          </p>
                        )}
                      </div>
                    ))}
                  </DescriptionExperience>
                  <ImageExperience data-aos="fade-up">
                    <img
                      src={experience.data.image.url}
                      alt={experience.data.image.alt}
                      height={experience.data.height}
                    />
                  </ImageExperience>
                </ExperienceContent>
              );
        ...
        <Technologies data-aos="fade-up">
          ...
          <ContainerTech>
            {technologies.map((tech) => (
              <Tech key={tech.uid} data-aos="fade-up">
                <img
        ...
        <ContactMe>
          <ContactImage data-aos="fade-up">
            <img
              src={textHeader.data.vector.url}
              alt={textHeader.data.vector.alt}
              height="400"
            />
          </ContactImage>
          <ContactContent data-aos="fade-up">
            <div>
              <img
                ...
                alt={textHeader.data.whatsappimage.alt}
                width="40"
              />
            ...
            <div>
              <img
                ...
                alt={textHeader.data.emailimage.alt}
                width="40"
              />
              <span>
              ...
              <li key={footer.uid}>
                <a href={footer.data.link.url} target="_blank">
                  <img
                    ...
                    alt={footer.data.image.alt}
                    width={footer.data.width}
                  />
                </a>
              </li>
            ))}
